...
| Contact | src/components/Contact.stories.tsx | Default, Embedded, WithInitialType |
...
| 3-6 | COMPONENT-OPTIMIZATION.md作成: 重複パターンリスト（判定基準付き） | docs/COMPONENT-OPTIMIZATION.md |
...
- FAQ/アコーディオン → `FAQ` コンポーネントまたは既存パターンを流用
- 電話番号リンク → 既存のSVG+リンクパターンを流用（`docs/COMPONENT-OPTIMIZATION.md` 参照）
- グラフ・チャート → `charts/` ディレクトリ内のコンポーネントを再利用
...
- `charts/` 系 - グラフ・チャート
- `useScrollAnimation` - スクロール連動アニメーション（src/hooks/useScrollAnimation.ts）
...
### COMPONENT-OPTIMIZATION.mdの重複判定基準

以下の基準で「重複」と判定し、共通化候補として記載する:
1. **見た目同一**: 2箇所以上で同一HTMLstructure/CSSクラスが使われている
...
### 未掲載7コンポーネント + 1フック（COMPONENTS.md追加対象）
...
| フック | Props | 説明 |
|-------|-------|------|
| useScrollAnimation | ref: RefObject, options?: { threshold?: number; rootMargin?: string } | スクロール連動アニメーション（Intersection Observer） |

### Contact.tsx 既存Props（Slice 3-2で文書化）

| Prop | 型 | デフォルト | 説明 |
|------|---|----------|------|
| initialContactType | string? | undefined | 初期選択される問い合わせ種別 |
| embedded | boolean | false | 埋め込みモード（影付きカード、セクション外観変更） |
| hideTitle | boolean | false | タイトル非表示（embeddedと組み合わせ使用）
...
8. docs/COMPONENT-OPTIMIZATION.mdに重複パターンと判定基準が記載されている

### デプロイ検証（Firebase Hosting）
9. `npm run build` でoutディレクトリが生成される（静的エクスポート確認）
10. storybook-staticがFirebase Hostingデプロイに含まれていないこと（firebase.jsonで除外確認）
