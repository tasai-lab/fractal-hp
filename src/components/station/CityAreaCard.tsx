  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm flex flex-col">
      ...
      <div className="p-4 md:p-5 flex flex-col flex-1">
        {/* 市名 */}
        <h4
          className="font-bold text-lg heading-gothic mb-3 pb-2 border-b border-[var(--color-sand)]"
        ...
        {areaData?.areaStats && (
          ...
          >
            <div className="text-center">
              ...
              >
                {areaData.areaStats.patients}<span className="text-sm ml-0.5">名</span>
              </p>
              <p className="text-xs text-[var(--color-ink-soft)] mt-0.5">利用者数</p>
            </div>
            <div className="text-center">
              <p
                className="text-xl md:text-2xl font-bold"
                style={{ color: areaData.theme.primary }}
              >
                {areaData.areaStats.monthlyVisits}<span className="text-sm ml-0.5">件</span>
              </p>
              <p className="text-xs text-[var(--color-ink-soft)] mt-0.5">月間訪問数</p>
            </div>
          </div>
        )}
        ...
        {/* 統計グリッド */}
        {areaData && (
          <div className="mt-auto border-t border-[var(--color-sand)]/50 pt-3">
        ...
        {detailHref && areaData && (
          <Link
            href={detailHref}
            className="group flex items-center justify-center gap-1 p-3 rounded-lg text-sm font-bold transition-colors mt-3 hover:gap-2"
            style={{ backgroundColor: `${areaData.theme.primary}15`, color: areaData.theme.primary }}
          >
            <span>詳しく</span>
